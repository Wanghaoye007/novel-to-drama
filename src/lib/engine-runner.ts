...
const engineStageProgress: Record<string, { progress: number; label: string }> = {
  ...
  final_apply_quality_policy: { progress: 85, label: "统一质量决策" },
  final_mark_human_review: { progress: 85, label: "标记人工复核" },
  state_commit_apply_quality_policy: { progress: 86, label: "校验状态写入" },
  state_commit_mark_human_review: { progress: 86, label: "标记人工复核" },
  next_round_context: { progress: 87, label: "写入下一轮上下文" },
};
