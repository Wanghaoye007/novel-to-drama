...
Reela / Reflix 短剧生产链最终质量的瓶颈在「输入文本」。用户/内部团队拿到的小说和脚本素材在体裁形态、剧本质量、频道题材、体量完整度四个维度上都严重参差不齐。直接灌入 Reela Planner 后产出参差不齐，靠 Bridge 层 prompt 模板和 Reela Planner 自身能力补不回来。

### 1.2 目标

做一个**内部工具**，把参差不齐的小说原料自动改编成符合 Reflix 对白剧标准、可直接用于 Reela 生产的脚本。

### 1.3 非目标

- 不替代 Reela Planner / Bridge 模板，本工具只生产「输入到 Reela 之前的标准化脚本」
- 不做面向 C 端的 SaaS 产品
- 不做爆款选题判断或热榜推荐（这是上游工作）
- 不做视频生成 / 镜头执行（这是下游 Reela 的职责）
...
Reela 内部生产 / 运营团队（小二、剧集运营、Bridge 模板团队自测）。

### 2.2 核心场景

**A 路（v0 范围）** 国内小说 → 国内 Reflix 对白短剧
小二拿到一本网文小说 → 上传工具 → 系统出 Bible 和分轮规划 → 小二审一下 Bible → 系统按轮次 5 集/批跑 → 小二审完一轮决定下一轮 → 跑完 N 轮导出 txt → 手动贴进 Reela 前端创建剧集。
...
- 重跑上限以外的可配置项
- 多剧种（仅 Reflix 对白剧）
- 自动接 Reela 后台 API 或 Planner JSON 输出

### 3.3 后续演进（已识别但不做）

- v1 增加 B 路 prompt（翻译 + 本地化映射表）
- v1 增加多剧种模板（古言 / 现代甜宠 / 暗托）
- v2 接 Reela Planner JSON schema 一键灌入
- v3 接 Reela API 直调
...
[用户确认页：可改 Bible]
        ↓
M3 轮次改编 worker（5 集/轮，顺序跑）   ← Reflix v8 prompt + 5 守 5 改；跨集传「前集摘要」；跨轮传「上轮摘要」
        ↓
M4 质量自查   ← v8 评审套路 + 9.0 阈值；不达标打红标，不自动重跑
        ↓
...
- M1 全新模块，DJ_Project 没做过
- M2 复用 DJ_Project pipeline 的 stage1+2+3（频道+人物小传+结构）
- M3 复用 reel-shows skill + Reflix prompt v8
- M4 复用 feedback_iterative_review_workflow（3 agent 并行 + 9.0 阈值）
- M5 复用 project_reflix_format_engine 格式转换 v5
...
prompt 模板路径
- 主 prompt 引用 reel-shows skill 的 Reflix v8 prompt（不复制，引用）
...
输入 M3+M4 通过的草稿
输出 符合 Reflix 原子 shot 格式的 txt（[SCENE] / [ACTION] / [SPEAKER]）
...
- [ ] 团队 2-3 个同事拿真实小说试用，反馈优于直接灌 Reela 当前流程
...
2. **Opus 4.7 改编质量未必稳定打 9.0** 现有 Reflix v8 prompt 在某些频道可能掉分
...
- 目标用户：内部团队（vs C 端 / 第三方 / PM 内测）
- 输出剧种：Reflix 对白剧 + 1-2 剧种，v0 实际只做 Reflix（vs 7 全 / 自动选）
- 功能边界：全链小说→多集脚本（vs 单集改编 / 仅格式化 / 模块自选）
- 输入痛点：四维度全参差（体裁、质量、频道、体量）
- 产品形态：独立 Web + vibecode（vs Reela 后台集成 / Skill 化）
- B 路本质：轻本地化，MVP 不重构（vs 翻译 / 重构 / 混合）
- MVP 起跑：先 A，B 留架构位（vs 仅 B / A+B 同上 / A 不留 B）
- 产出单位：轮次制 5 集/轮，多轮跑（vs 一次一集 / 全量 / 双模式）
- 交付接口：txt 导出，手动贴 Reela 前端（vs docx / Planner JSON / API）
...
- 木木 fixture 小说 `/Users/wangzipeng/Documents/DJ_Project/木木给的脚本/`
- 脚本格式转换 v5 `~/Desktop/脚本格式转换引擎.txt`
- 已落地 skills `reel-shows` / `duanju-gaiban` / `reela-cli-usage`
- Reflix prompt v8 设计记忆 `~/.claude/projects/-Users-wangzipeng/memory/project_reflix_prompt.md`
