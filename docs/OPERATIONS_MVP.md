...
- 当前运营环境使用真实模型：OpenRouter 豆包 `bytedance-seed/seed-2.0-mini`，按集顺序生成并传递上集 handoff。
- 默认且唯一的生产改编链路为「强剧情优先」：Web 任务只调用 Python Engine 的当前生产流程，运营只需要选择模型、轮次集数和修复预算。
...
OPENAI_MODEL=bytedance-seed/seed-2.0-mini
